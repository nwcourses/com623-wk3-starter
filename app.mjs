...
app.use((req, res, next) => {
    if(["POST", "PUT", "DELETE"].indexOf(req.method) != -1 && !req.session.username) {
...
app.put('/song/:id', (req, res) => {
    try {
        if(!req.body.title || !req.body.artist || !req.body.year) {
            ...
            const stmt = db.prepare("UPDATE wadsongs SET title=?,artist=?,year=? WHERE id=?");
            const info = stmt.run(req.body.title, req.body.artist, req.body.year, req.body.id);
